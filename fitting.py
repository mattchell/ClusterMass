...
class FittingSignal:
    ...
    def chi_sqrd(self, A, del_z_bar, sep, Cov_mat):
        if A > 0:
            model_d_z_b = self.model_delta_z_bar(A, sep)
        elif A == 0.:
            model_d_z_b = np.zeros(sep.size)
        c_s = float(np.matrix(model_d_z_b-del_z_bar)*Cov_mat.I*np.matrix(model_d_z_b-del_z_bar).T)
        return c_s


    # fitting the data to the model using chi-squared
    def fit_for_A(self, sep, del_z_bar, N, b_g_rel, c_rel, n, Cov_mat, covar_type, nbins):
        self.c_rel = c_rel
        self.get_quad_coeff_for_alpha(b_g_rel, n)
        A_step, dA = np.linspace(0., 10., 199, endpoint=True, retstep=True)
        prob_step = np.asarray([np.exp(-0.5*self.chi_sqrd(A_i, del_z_bar, sep, Cov_mat)) for A_i in A_step])
        ...
        plt.savefig('auto/probability/Prob_A_{0}_{1}.png'.format(n, covar_type))
        plt.close()
        ...
        print("   Chi^2 of fit: {0}".format(self.chi_sqrd(exp_A,del_z_bar,sep,Cov_mat)))
    ...
    def get_quad_coeff_for_alpha(self, b_g_rel, n):
        try:
            quad_coeff = err.read_data("data/quadradic_fit_to_alpha_{0}.txt".format(n))
        except IOError:
    ...
    # alpha from model
    def alpha(self, z_c):
        return self.quad_coeff[0]*np.power(z_c-0.215,2.)+self.quad_coeff[1]*(z_c-0.215)+self.quad_coeff[2]


    # calculate expectation value of signal over the sample using
    def model_delta_z_bar(self, A, Rs, lazy=False):
        if lazy:
            delta_z_bar_for_bins = np.asarray([[self.alpha(self.z_step_c[i])*self.Sigma_of_cluster(self.z_step_c[i], self.lamb_step[j], A, Rs) for j in range(self.lamb_step.size)] for i in range(self.z_step_c.size)])
            return mid_integral(np.asarray([self.dz_c, self.dlamb]), self.p_c[:,:,np.newaxis]*delta_z_bar_for_bins)
        else:
            delta_z_bars = np.asarray([self.alpha(clust[2])*self.Sigma_of_cluster(clust[2], clust[4], A, Rs) for clust in self.c_rel.T ])
            return np.average(delta_z_bars, axis=0)
