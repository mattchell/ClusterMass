...
print('n = {0}\nR_min = {1}\nR_max = {2}'.format(n, min_sep, max_sep))
...
fitting_NFW.set_p_of_zr(c_rel.copy(),20.,210.)
...
N = np.zeros(nbins)
cat_g = treecorr.Catalog(ra=b_g_rel[0].copy(), dec=b_g_rel[1].copy(), w=np.power(b_g_rel[2].copy(),n), ra_units='deg', dec_units='deg')
nn.process(cat_c,cat_g)

z_bar = nn.weight.copy()/nn.npairs.copy()
mean_z = np.average(np.power(b_g_rel[2],n))
print(mean_z, z_bar, n)
...
Cov_mat = err.jack_knife(nn,b_g_rel[0:3].copy(),c_rel.copy(),mean_z,sep,nbins,n,ncen=70,read_from_file=False)
#Cov_mat = err.randoms_cluster_errors(n_clusters,clust_z_min,clust_z_max,nn,cat_g,mean_z,sep,nbins,read_from_file=False)
#Cov_mat = err.anylitical_error(b_g_rel[2],N)
...
M_0, var_M_0 = fitting_NFW.multiple_profiles(sep.copy(),del_z_bar.copy(),N.copy(),b_g_rel.copy(),c_rel.copy(),n,Cov_mat.copy())
...
quad_coeff = fitting_NFW.get_quad_coeff_for_d_del_z_d_Sig(b_g_rel.copy(),n)
for r_i in r:
    model.append(fitting_NFW.expect_delta_z_bar_of_Sig(M_0,r_i,quad_coeff))
    model_p.append(fitting_NFW.expect_delta_z_bar_of_Sig(M_p,r_i,quad_coeff))
    model_m.append(fitting_NFW.expect_delta_z_bar_of_Sig(M_m,r_i,quad_coeff))
...
data_graph = plt.errorbar(sep, del_z_bar, yerr=sig, fmt='o', color='b', capsize=3)
...
#model_p_graph = plt.plot(r,model_p,'r',alpha=0.5)
#model_m_graph = plt.plot(r,model_m,'r',alpha=0.5)
if n == 1:
    plt.ylabel(r'$\Delta \bar{z}$', fontsize=14)
else:
    plt.ylabel(r'$\Delta \bar{z^{'+str(n)+'}}$', fontsize=14)
...
plt.legend([data_graph, (model_graph, sig_label)], [r'$\Delta \bar{z}$ Signal', 'NSF model'])
#plt.suptitle('Redshift Signal vs. Physical Radius')
plt.savefig("auto/delta_z_bar_fit_NFW_{0}.png".format(n))
plt.close()

# ploting r*avg redshift as a function of r
plt.figure(1,figsize=(7.5,7),dpi=80)
plt.axhline(y=0.0, xmin=0.0, xmax=1.0, linewidth=1, color='k')
data_graph = plt.errorbar(sep, sep*del_z_bar, yerr=sep*sig, fmt='o', color='b', capsize=3)
...
if n == 1:
    ...
    plt.ylabel(r'$R \Delta \bar{z^{'+str(n)+'}} \ (Mpc)$', fontsize=14)
...
plt.subplots_adjust(hspace=0.26,wspace=0.56,right=0.95,left=0.16,top=0.93,bottom=0.15)
plt.legend([data_graph, (model_graph, sig_label)], [r'$\Delta \bar{z}$ Signal', 'NSF model'])
#plt.suptitle('Redshift Signal Times Radius vs. Physical Radius')
plt.savefig("auto/delta_z_bar_r_fit_NFW_{0}.png".format(n))
plt.close()
