# Dealing with fitting and signal and noise information for (r-i) bins
# (c) mattchelldavis
...
def find_d_delta_z_bar_d_mu(b_g_rel,avg_z_in_bin,n,ri_bin_bounds):
    a_s = np.zeros(n)
    a_sigs = np.zeros(n)
    ...
    var = np.zeros((10,n))
    for i in range(10):
        z_bar_i,var_i = calc_delta_z_bar_of_mu(b_g_rel,avg_z_in_bin,mu[i],n,ri_bin_bounds)
        var[i] = var_i
        z_bar_mu[i] = z_bar_i
...
d_del_d_mu = [0.0332980199393,np.asarray([0.0357189573687,0.0357189573687]),np.asarray([0.0443678367999,0.0320661910019,0.0341763796703])]
