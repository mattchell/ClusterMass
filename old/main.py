...
# binning data by i band magnitude
...
fitting.set_p_of_c(c_rel[2],clust_z_min,clust_z_max)
fitting.set_p_of_g_binned(b_g_rel_binned,n,gal_z_min)

# fitting.find_d_delta_z_bar_d_mu(b_g_rel,avg_z_in_bin,n,i_bin_bounds)
...
A, var_A = fitting.multiple_profiles(sep,del_z_bar,N,b_g_rel_binned,c_rel[2],n)
...
if int(np.power(n,0.5)) == np.power(n,0.5):
    ...
    if n <= small_n*big_n:
        ...
        row = big_n
        col = big_n
model = fitting.delta_z_bar_of_mu(model_mu,n)
model_p = fitting.delta_z_bar_of_mu(model_mu_p,n)
model_m = fitting.delta_z_bar_of_mu(model_mu_m,n)
for k in range(n):
